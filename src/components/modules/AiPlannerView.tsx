...
export const AiPlannerView: React.FC = () => {
  ...
  return (
    <div className="animate-fadeIn" style={{ padding: "24px", maxWidth: "1400px", margin: "0 auto", display: "flex", flexDirection: "column", gap: "24px", fontFamily: "Inter, sans-serif" }}>
      ...
      >
        <div style={{ flex: 1 }}>
          <div style={{ display: "flex", alignItems: "center", gap: "10px", marginBottom: "10px" }}>
            ...
            <span style={{ fontSize: "10px", color: "rgba(255,255,255,0.55)" }}>Generative PMO Engine</span>
          </div>
      ...
      {/* AI Prompt Launcher */}
      <div className="glass-card rounded-xl p-5 shadow-sm space-y-3">
                ...
                {isExp && (
                  <div className="p-4 space-y-4">
                    {phase.workPackages.map((wp) => (
                      <div key={wp.id} className="border border-[var(--border)] rounded-lg p-3.5 bg-[var(--bg-card)] space-y-3 shadow-xs">
                        <div className="flex items-center justify-between border-b border-[var(--border)] pb-2">
                          ...
                          <span className="text-[10px] text-[var(--text-secondary)]">Owner: <strong className="text-[var(--text-primary)]">{wp.owner}</strong></span>
                        </div>
                          ...
                          {wp.tasks.map((t) => (
                            <div key={t.id} className="bg-[var(--bg-card)] p-2.5 rounded-md border border-[var(--border)] text-xs flex flex-col md:flex-row md:items-center justify-between gap-2">
                              <div className="space-y-1">
                                <div className="flex items-center gap-2">
                                  <span className="font-mono text-[10px] text-[var(--accent)] font-bold">{t.wbsCode}</span>
                                  <span className="font-semibold text-[var(--text-primary)]">{t.title}</span>
                                  {t.isCriticalPath && (
                                    <span className="text-[9px] bg-[var(--pink-dim)] text-[var(--pink)] font-bold px-1.5 py-0.5 rounded border border-red-200">
                                      Critical Path
                                    </span>
                                  )}
                                  {t.isMilestone && (
                                    <span className="text-[9px] bg-[var(--amber-dim)] text-[var(--amber)] font-bold px-1.5 py-0.5 rounded border border-amber-200">
                                      Milestone
                                    </span>
                                  )}
                                </div>
