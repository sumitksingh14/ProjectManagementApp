    ...
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-xl flex items-center justify-between">
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-xs text-slate-400">Earned Value Management (EVM) Standard</span>
          </div>
          <h1 className="text-2xl font-bold text-white">Financial & EVM Analytics</h1>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 text-xs">
        <div className="bg-slate-900 border border-slate-800 p-5 rounded-xl shadow-md space-y-2">
          <span className="text-slate-400 font-medium">Cost Performance Index (CPI)</span>
          <div className="text-3xl font-extrabold text-white font-mono">{evm.CPI}</div>
          <p className="text-[10px] text-slate-400">Formula: EV / AC ({evm.CPI >= 1 ? "Under Budget / Efficient" : "Over Budget"})</p>
        </div>

        <div className="bg-slate-900 border border-slate-800 p-5 rounded-xl shadow-md space-y-2">
          <span className="text-slate-400 font-medium">Schedule Performance Index (SPI)</span>
          <div className="text-3xl font-extrabold text-white font-mono">{evm.SPI}</div>
          <p className="text-[10px] text-slate-400">Formula: EV / PV ({evm.SPI >= 1 ? "Ahead of Schedule" : "Behind Schedule"})</p>
        </div>

        <div className="bg-slate-900 border border-slate-800 p-5 rounded-xl shadow-md space-y-2">
          <span className="text-slate-400 font-medium">Estimate at Completion (EAC)</span>
          <div className="text-3xl font-extrabold text-indigo-400 font-mono">${(evm.EAC / 1000).toFixed(0)}k</div>
          <p className="text-[10px] text-slate-400">Formula: BAC / CPI (Baseline: ${(evm.BAC / 1000).toFixed(0)}k)</p>
        </div>

        <div className="bg-slate-900 border border-slate-800 p-5 rounded-xl shadow-md space-y-2">
          <span className="text-slate-400 font-medium">Variance at Completion (VAC)</span>
          <div className="text-3xl font-extrabold text-emerald-400 font-mono">+${(evm.VAC / 1000).toFixed(0)}k</div>
          <p className="text-[10px] text-slate-400">Formula: BAC - EAC (Favorable cost saving)</p>
        </div>
      </div>
