      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-xl flex items-center justify-between">
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-xs text-slate-400">PMO Stakeholder Governance</span>
          </div>
          <h1 className="text-2xl font-bold text-white">Stakeholder Management</h1>
      ...
      {showForm && (
        <form onSubmit={handleAddStakeholder} className="bg-slate-900 border border-indigo-500/40 rounded-2xl p-6 shadow-2xl space-y-4 text-xs">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div>
              <label className="block text-slate-400 mb-1">Influence Power</label>
              <select
            ...
            <div>
              <label className="block text-slate-400 mb-1">Interest Level</label>
              <select
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              {keyPlayers.map((s) => (
                <div key={s.id} className="bg-slate-900/80 p-2.5 rounded-lg border border-indigo-500/20 flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-[10px] text-slate-400">{s.position} ({s.department})</p>
                  </div>
                  <span className="text-[10px] bg-indigo-600/40 text-indigo-200 px-2 py-0.5 rounded border border-indigo-500/40">
          ...
          <div className="bg-amber-950/30 border border-amber-500/30 p-4 rounded-xl space-y-2">
            ...
            <div className="space-y-2 text-xs">
              {keepSatisfied.length > 0 ? (
                ...
                <p className="text-[11px] text-slate-500 italic">No stakeholders currently mapped in this quadrant.</p>
              )}
            </div>
          </div>
          ...
          <div className="bg-slate-800/40 border border-slate-700/60 p-4 rounded-xl space-y-2">
            <div className="flex items-center justify-between border-b border-slate-700/60 pb-2">
              <span className="text-xs font-bold text-slate-300 uppercase tracking-wider">Monitor (Minimal Effort)</span>
            ...
            <tbody className="divide-y divide-slate-800/60">
              {stakeholders.map((s) => (
                <tr key={s.id} className="hover:bg-slate-800/40 transition-colors">
                  ...
                  <td className="p-3 text-slate-300">{s.communicationPreference}</td>
                  <td className="p-3 text-slate-400 max-w-xs leading-relaxed">{s.engagementStrategy}</td>
                </tr>
              ))}
            </tbody>
          </table>
